...
func (rad Radians) Degrees() Degrees { return Degrees(rad * (180.0 / Pi)) } //rad_to_deg
...
func (angle Radians) Vector2() Vector2 { // Vector2.from_angle
...
func (deg Degrees) Radians() Radians { return Radians(deg * (Pi / 180.0)) } //deg_to_rad

// Vector2 creates a unit Vector2 rotated to the given angle in degrees. This is equivalent to
// doing angle.Radians().Vector2()
func (deg Degrees) Vector2() Vector2 { //Vector2.from_angle
	angle := deg.Radians()
	return Vector2{float(math.Cos(float64(angle))), float(math.Sin(float64(angle)))}
}
...
func (e EulerAngles) Quaternion() Quaternion { //Quaternion.from_euler
	var (
		half_a1 = e[Y] * 0.5
		half_a2 = e[X] * 0.5
		half_a3 = e[Z] * 0.5
...
func (e EulerAngles) Basis(order EulerOrder) Basis { //Basis.from_euler
	var c, s float
