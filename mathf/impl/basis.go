...
func NewBasisScaledBy(scale Vector3) Basis { //Basis.from_scale
	return Basis{
		{scale[X], 0, 0},
		{0, scale[Y], 0},
		{0, 0, scale[Z]},
...
func NewBasisRotatedAround(axis Vector3, angle Radians) Basis { //Basis(Vector3,float)
	var rows Basis
...
func NewBasisLookingAt(target, up Vector3, use_model_front bool) Basis { //Basis.looking_at
...
func (b Basis) Determinant() float64 { //Basis.determinant
	return float64(b[0][0]*(b[1][1]*b[2][2]-b[2][1]*b[1][2]) -
...
func (b Basis) EulerAngles(order EulerOrder) EulerAngles { //Basis.get_euler
	switch order {
	case EulerOrderXYZ:
...
func (b Basis) Quaternion() Quaternion { //Basis.get_rotation_quaternion
...
func (b Basis) Scale() Vector3 { //Basis.get_scale
	...
	var det_sign = Signf(b.Determinant())
	return Vector3{
...
func (b Basis) Inverse() Basis { //Basis.inverse
	var co = [3]float{
...
func (b Basis) IsConformal() bool { //Basis.is_conformal
	var (
		x = b[0]
		y = b[1]
		z = b[2]
...
func (b Basis) IsApproximatelyEqual(other Basis) bool { //Basis.is_equal_approx
...
func (b Basis) IsFinite() bool { //Basis.is_finite
...
func (b Basis) Orthonormalized() Basis { //Basis.orthonormalized
	var (
		x = b[0].Normalized()
...
// The axis must be a normalized vector.
func (b Basis) Rotated(axis Vector3, angle Radians) Basis { //Basis.rotated
...
func (b Basis) Scaled(scale Vector3) Basis { //Basis.scaled
	b[0][0] *= scale[X]
	b[0][1] *= scale[X]
	b[0][2] *= scale[X]
	b[1][0] *= scale[Y]
...
func (b Basis) Slerp(to Basis, weight float64) Basis { return b.lerp(to, weight) } //Basis.slerp

// TransposedDotX returns the transposed dot product with the X axis of the matrix.
func (b Basis) TransposedDotX(v Vector3) float64 { //Basis.tdotx
	return float64(b[0][0]*v[0] + b[1][0]*v[1] + b[2][0]*v[2])
}

// TransposedDotY returns the transposed dot product with the Y axis of the matrix.
func (b Basis) TransposedDotY(v Vector3) float64 { //Basis.tdoty
	return float64(b[0][1]*v[0] + b[1][1]*v[1] + b[2][1]*v[2])
}

// TransposedDotZ returns the transposed dot product with the Z axis of the matrix.
func (b Basis) TransposedDotZ(v Vector3) float64 { //Basis.tdotz
	return float64(b[0][2]*v[0] + b[1][2]*v[1] + b[2][2]*v[2])
}

// Transposed returns the transposed version of the matrix.
func (m Basis) Transposed() Basis { //Basis.transposed
	return Basis{
		{m[0][0], m[1][0], m[2][0]},
		{m[0][1], m[1][1], m[2][1]},
		{m[0][2], m[1][2], m[2][2]},
...
func (b Basis) Mul(other Basis) Basis { //Basis * Basis
	return Basis{
...
func (m Basis) Transform(v Vector3) Vector3 { //Basis * Vector3
	return Vector3{
		float(m[0].Dot(v)),
		float(m[1].Dot(v)),
		float(m[2].Dot(v)),
