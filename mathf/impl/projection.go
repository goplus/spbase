...
func NewProjectionWithDepthCorrection(fliy bool) Projection { //Projection.create_depth_correction
	return Projection{
		Vector4{1, 0, 0, 0},
...
func NewProjectionForHeadMountedDisplay(eye int, aspect, intraocular_dist, display_width, display_to_lens, oversample, z_near, z_far float64) Projection { //Projection.create_for_hmd
...
func NewProjectionWithFrustum(left, right, bottom, top, z_near, z_far float64) Projection { //Projection.create_frustum
	return Projection{
...
func NewProjectionWithFrustumAspectRatio(size, aspect float64, offset Vector2, z_near, z_far float64, flip_fov bool) Projection { //Projection.create_frustum_aspect
	if !flip_fov {
		size *= aspect
	}
	return NewProjectionWithFrustum(-size/2+float64(offset[X]), size/2+float64(offset[X]), -size/aspect/2+float64(offset[Y]), size/aspect/2+float64(offset[Y]), z_near, z_far)
}
...
// clipping planes.
func NewProjectionThatIsOrthogonal(left, right, bottom, top, z_near, z_far float64) Projection { //Projection.create_orthogonal
	return Projection{
...
//
// flip_fov determines whether the projection's field of view is flipped over its diagonal.
func NewProjectionThatIsOrthogonalWithAspectRatio(size, aspect, z_near, z_far float64, flip_fov bool) Projection { //Projection.create_orthogonal_aspect
	if !flip_fov {
		size *= aspect
	}
...
//
// flip_fov determines whether the projection's field of view is flipped over its diagonal.
func NewProjectionWithPerspective(fovy Degrees, aspect, z_near, z_far float64, flip_fov bool) Projection { //Projection.create_perspective
	if flip_fov {
...
//
// flip_fov determines whether the projection's field of view is flipped over its diagonal
func NewProjectionWithPerspectiveForHeadMountedDisplay(fovy Degrees, aspect, z_near, z_far float64, flip_fov bool, eye int, intraocular_dist, convergence_dist float64) Projection { //Projection.create_perspective_hmd
	if flip_fov {
		fovy = ProjectionFovy(fovy, 1.0/aspect)
	}

	var left, right, ymax, xmax, frustumshift float64
	var modeltranslation float

	ymax = z_near * float64(Tan((fovy / 2.0).Radians()))
	xmax = ymax * aspect
...
func ProjectionFovy(hfov Degrees, aspect float64) Degrees { //Projection.get_fovy
...
func (p Projection) Determinant() float64 { //Projection.determinant
	return float64(p[0][3]*p[1][2]*p[2][1]*p[3][0] - p[0][2]*p[1][3]*p[2][1]*p[3][0] -
...
func (p Projection) FlippedY() Projection { //Projection.flipped_y
	for i := range p[0] {
		p[1][i] = -p[1][i]
	}
	return p
...
func (p Projection) AspectRatio() float64 { //Projection.get_aspect
	vp_he := p.ViewportHalfExtents()
	return float64(vp_he[X] / vp_he[Y])
}

// FarPlaneHalfExtents returns the dimensions of the far clipping plane of the projection, divided by two.
func (p Projection) FarPlaneHalfExtents() Vector2 { //Projection.get_far_plane_half_extents
...
func (p Projection) FieldOfView() Degrees { //Projection.get_fov
	m := (*[16]float)(unsafe.Pointer(&p[0][0]))
	var right_plane = Plane{
		Normal: Vector3{
			m[3] - m[0],
...
// this Projection.
func (p Projection) LevelOfDetailMultiplier() float64 { //Projection.get_lod_multiplier
	if p.IsOrthogonal() {
		return float64(p.ViewportHalfExtents()[X])
	}
	var zn = p.NearZ()
	var width = float64(p.ViewportHalfExtents()[X]) * 2.0
	return 1.0 / (zn / width)
}
...
func (p Projection) PixelsPerMeter(pixel_width int64) int64 { //Projection.get_pixels_per_meter
	var result = Vector4{1, 0, -1, 1}.Transform(p)
	return int64((result[X]*0.5 + 0.5) * float(pixel_width))
}
...
// PLANE_BOTTOM.
func (p Projection) ProjectionPlane(plane ProjectionPlane) (new_plane Plane) { //Projection.get_projection_plane
	m := (*[16]float)(unsafe.Pointer(&p[0][0]))
	switch plane {
	case PlaneNear:
		new_plane = Plane{
...
// divided by two.
func (p Projection) ViewportHalfExtents() Vector2 { //Projection.get_viewport_half_extents
	m := (*[16]float)(unsafe.Pointer(&p[0][0]))
	near_plane := Plane{
		Normal: Vector3{
			m[3] + m[2],
...
func (p Projection) FarZ() float64 { //Projection.get_z_far
	m := (*[16]float)(unsafe.Pointer(&p[0][0]))
	var new_plane = Plane{
		Normal: Vector3{
			m[3] - m[2],
...
func (p Projection) Inverse() Projection { //Projection.inverse
	var i, j int
...
func (p Projection) IsOrthogonal() bool { return p[3][3] == 1.0 } //Projection.is_orthogonal
...
func (p Projection) JitterOffseted(jitter Vector2) Projection { //Projection.jitter_offseted
	p[3][0] += jitter[X]
	p[3][1] += jitter[Y]
	return p
}
...
func (p Projection) PerspectiveAdjustedNearZ(new_near_z float64) Projection { //Projection.perspective_znear_adjusted
	var zfar = p.FarZ()
...
func (p Projection) Mul(other Projection) Projection { //Projection * Projection
